...
class PromptDataSyncer:
    ...
    def export_database_to_json(self, output_file: Path = None) -> str:
        """Export current database to consolidated JSON format."""
        if not output_file:
            output_file = self.base_path / f"consolidated_prompts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def update_production_exports(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for level in ['L1', 'L2', 'L3', 'L4', 'L5']:
                ...
                complexity_data[level] = [dict(row) for row in cursor.fetchall()]
...
"""
    ...
    def full_sync(self, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            populator = PromptDatabasePopulator(self.db_path, self.sequential_think_path)
...
def main():
    ...
    print(f"📊 Change Detection: {len(changes['modified_files'])} files modified")
    ...
    if changes['database_outdated'] or input("\nForce full sync? (y/N): ").lower() == 'y':
        ...
        if 'quality_improvements' in results:
            ...
            print(f"  • Structure improved: {improvements['structure_improved']}")
            print(f"  • Domains standardized: {improvements['domain_standardized']}")
