      ...
      ${prompts
        ...
        <div class="prompt-card">
          ...
          <div class="card-footer">
            ...
            <a href="https://github.com/${contributor.replace(
              "@",
              ""
            )}" class="contributor-badge" target="_blank" rel="noopener">${contributor}</a>
          </div>
        </div>
      `
        )
...
async function renderSidebarPrompts() {
  ...
  if (searchResults) {
    searchResults.innerHTML = prompts
      .map(
        ({ app }) => `
...
function showModal(app, prompt, contributor) {
  ...
  if (!modalOverlay) {
    ...
    const modalCopyButton = modalOverlay.querySelectorAll(
    ...
    modalCopyButton.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (button.classList.contains("modal-chat-button")) {
          alert(
            "Now you can paste the prompt into your AI IDE, deeplinks to AI IDEs are coming soon (I hope)! — IDE devs, please DM me!"
          );
        }
      });
    });
